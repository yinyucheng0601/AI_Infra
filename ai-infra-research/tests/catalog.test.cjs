...
test('19 manually curated entries after explicit exclusions',()=>{
 assert.equal(assets.length,19);assert.equal(assets.filter(a=>a.sources[0].repository==='pto').length,10);
 assert.equal(new Set(assets.map(a=>a.id)).size,19);
